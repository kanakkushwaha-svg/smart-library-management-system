...
SET issue_date = CURDATE(),
    ...
    SHOW CREATE TABLE issued_books;
    
    ALTER TABLE issued_books
DROP INDEX unique_issue;

SELECT COUNT(*) FROM issued_books;



SELECT student_roll_no, book_id, issue_date, COUNT(*) AS times
FROM issued_books
GROUP BY student_roll_no, book_id, issue_date
HAVING COUNT(*) > 1;
...
CREATE TRIGGER prevent_duplicate_active_issue
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM issued_books
        WHERE student_roll_no = NEW.student_roll_no
          AND book_id = NEW.book_id
          AND return_date IS NULL
    ) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'This student already has this book issued';
    END IF;
END$$
